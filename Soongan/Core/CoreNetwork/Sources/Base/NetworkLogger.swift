        ...
        case .success:
            ...
            if let data = response.data,
               ...
               let prettyPrintedString = String(data: prettyData, encoding: .utf8) {
                ...
                print("Success response body is not valid JSON or is empty.")
            }
        ...
        case .failure(let error):
            ...
            if let data = response.data {
                do {
                    ...
                    print("Code: \(errorResponse.statusCode)")
                    print("Message: \(errorResponse.message)")
                    print("detailMessage: \(errorResponse.detailMessage)")
                } catch {
                    print("💀 Unable to decode APIErrorResponse. Raw response:")
                    if let errorString = String(data: data, encoding: .utf8) {
                        print(errorString)
                    } else {
                        print("(Could not convert error data to string)")
                    }
                }
            } else {
